...
function MyBag(props) {
  ...
  const checkTax = () => {
    if (typeof mycart !== "undefined") {
      // const price = mycart.map(item => item.quantity * item.product_selling_price).reduce((a,b) => a + b);
      const tax = mycart
  ...
  const checkDeliveryCharges = () => {
    // if (typeof mycart !== "undefined") {
    //   // const price = mycart.map(item => item.quantity * item.product_selling_price).reduce((a,b) => a + b);
    //   const mainWareHouseDeliveryCharges = mycart.map(
    //     (item) => item.main_warhouse_delivery_charge
    //   );
    //   if (
    //     typeof mainWareHouseDeliveryCharges !== "undefined" ||
    //     mainWareHouseDeliveryCharges !== "null"
    //   ) {
    //     const deliveryCharges = mycart
    //       .map((item) => item.main_warhouse_delivery_charge * item.quantity)
    //       .reduce((a = 0, b = 0) => a + b, 0);
    ...
    //   ) {
    //     const deliveryCharges = mycart
    //       .map((item) => item.seller_delivery_charge * item.quantity)
    //       .reduce((a = 0, b = 0) => a + b, 0);
    //     return Number(deliveryCharges);
    //   }
    // }
  ...
  const placeOrder = () => {
    if (typeof mycart !== "undefined") {
      ...
      order.cart_total = orderTotal() + checkDeliveryCharges() + checkTax().toFixed(2)
      ...
      <div className={`${classes.cart}`}>
        ...
        {`${_.map(mycart || []).length}` > 0 ? (
          <div
            ...
            style={{
              marginTop: `${_.map(mycart || []).length}` === 0 ? "10%" : "5%", marginLeft:"10%"
            }}
          >
            <h5>
              <strong>
                ...
                <span style={{ color: "#c82557" }}>-----</span> عنوان{" "}
        ...
        {`${_.map(mycart || []).length}` > 0 ? (
          <div className={`${classes.cartContainer} cartContainer`}>
            <div className={`${classes.leftContainer} leftContainer`}>
              ...
              >
                <strong>
                  حقيبة التسوق الخاصة بي ({_.map(mycart || []).length}{" "}
                  {`${_.map(mycart || []).length}` <= 1 ? "بند" : "العناصر"})
                </strong>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <span>{item.product_translated_name}</span>
                          <span
                            style={{
                              textAlign: "right",
                              marginLeft: "1rem",
                          ...
                          >
                            ...
                            <span style={{ marginLeft: "1rem" }}>
                              {item.available_size
                                ...
                                .map((item) => item.translated_size_value)}
                            </span>
                          </span>
                          ...
                          >
                            ...
                            <span style={{ textTransform: "capitalize" }}>
                              {item.available_color
                                ...
                                .map((item) => item.translated_color_value)}
                            </span>
                          </span>
                        </div>
                        <br />
                        ...
                        <div>
                          ...
                          >
                            <DeleteIcon className="svg-icon" /> إزالة
                            </Button>
                          ...
                          {localStorage.getItem("user_token") ?
                            ...
                            >
                              <FavoriteIcon className="svg-icon" /> الانتقال إلى قائمة الرغبات
                            </Button>
                            :
                            <Link
                              to={{
            ...
            <div className={`${classes.rightContainer} rightContainer`}>
              <div style={{ margin: 1 }}>
                ...
                <div className={classes.rightContainerItem}>
                  <p className="para">إجمالي الحقيبة</p>
                  <p className="para">SAR {checkPrice()}</p>
                </div>
                <div className={classes.rightContainerItem}>
                  <p className="para">خصم</p>
                  <p className="para">
                    SAR {checkCoupon ? checkDiscount() : 0}
                  </p>
                </div>
                <div className={classes.rightContainerItem}>
                  <p className="para">ضريبة</p>
                  <p className="para">SAR {checkTax()}</p>
                </div>
                <div className={classes.rightContainerItem}>
                  <p className="para">الطلب الكلي</p>
                  <p className="para">SAR {orderTotal() + checkTax()}</p>
                </div>
                <div className={classes.rightContainerItem}>
                  <p className="para">رسوم التوصيل</p>
                  <p className="para">{checkDeliveryCharges()}</p>
                </div>
                <hr />
                <div className={classes.rightContainerItem}>
                  ...
                  <p style={{ fontWeight: "bold" }} className="para">
                    SAR {orderTotal() + checkDeliveryCharges() + checkTax()}
                  </p>
                </div>
            ...
            >
              <div style={{ marginTop: "2%" }}>
                <div className={classes.emptyBagTitle}>حقيبتك فارغة</div>
